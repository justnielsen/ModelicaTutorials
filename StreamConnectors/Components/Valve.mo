...
model Valve "Simple model of a valve"
  parameter Real K=m_flow_nominal/(sqrt(dp_nominal)*u_nominal)
    "Pressure drop coefficient";
  parameter Real dp_nominal=0.5
    "Nominal pressure drop (at opening = 'u_nominal') [bar]"
    annotation (Dialog(group="Nominal values"));
  parameter Real m_flow_nominal=1
    "Nominal mass flow rate (at opening = 'u_nominal') [kg/s]"
    annotation (Dialog(group="Nominal values"));
  ...
  Modelica.Blocks.Interfaces.RealInput u annotation (Placement(
        transformation(extent={{-228,38},{-188,78}}), iconTransformation(
        extent={{-20,-20},{20,20}},
        rotation=270,
        origin={0,80})));
equation
  ...
  annotation (
    ...
    Icon(coordinateSystem(preserveAspectRatio=false), graphics={Polygon(
          ...
          fillPattern=FillPattern.Solid), Line(points={{0,60},{0,0}}, color=
             {0,0,0})}),
